...
def repo_reset_and_clean_checkout(commit_hash: str, logger=None) -> None:
    ...
    # 新增初始化git命令；
    remove_cmd = ["rm", "-rf", ".git"]
    init_cmd = ["git", "init"]
    add_all_cmd = ["git", "add", "."]
    commit_cmd = ["git", "commit", "-m", "Temporary commit for storing changes"]
    print('[Running command] rm -rf .git')
    run_command(logger, remove_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    print('[Running command] git init')
    run_command(logger, init_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    print('[Running command] git add .')
    try:
        run_command(logger, add_all_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except:
        if os.path.exists("astropy_helpers"):
            shutil.rmtree("astropy_helpers")
        run_command(logger, add_all_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    print('[Running command] git commit -m Temporary commit for storing changes')
    run_command(logger, commit_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    # repo_commit_current_changes(logger)
    # end
    ...
    # run_command(logger, submodule_unbind_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def run_string_cmd_in_conda(
    ...
    conda_cmd = (
        ...
        f"source {conda_script_path} ; conda activate {env_name} ; {command}"
    )
...
def remove_temp_dir(dir_path: str):
    """
    Remove the directory if it contains 'temp' in its path.
    Args:
        dir_path (str): Path to the directory.
    """
    if 'temp' in dir_path:
        if os.path.exists(dir_path):
            try:
                ...
                print(f"Temporary directory {dir_path} removed.")
            except Exception as e:
                print(f"An error occurred while trying to remove {dir_path}: {e}")
        else:
            print(f"Directory {dir_path} does not exist.")
    else:
        print(f"The directory {dir_path} does not contain 'temp' in its path and will not be removed.")
