...
class MessageThread(object):
    ...
    def remove_developer_patch(self):
        for i, msg in enumerate(self.messages):
            if msg["role"] == "user" and "<attention>" in msg["content"]:
                begin = msg["content"].find("<attention>")
                end = msg["content"].find("</issue>")
                self.messages[i]['content'] = self.messages[i]['content'].replace(msg["content"][begin:end], "")
                break
