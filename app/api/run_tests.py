...
def txt_file_contains_string(path_to_txt, expected_output, other_patterns=[]):
    ...
    try:
        with open(path_to_txt, "r", encoding="utf-8") as file:
            ...
            # print('[1]. Debugging verify bugs!')
            # print('[2]. content_last_500', content[-500:])
            # print('[3]. filtered_content_last_500', filtered_content[-500:])
            # print('[4]. other_patterns', other_patterns)
            # print('[5]. expected_output', expected_output)
            for pattern in other_patterns:
                if pattern in filtered_content:
                    # print(f'[6]. results [False]. Found error: {pattern} in the output')
                    return False
            # print(f'[6]. results [{expected_output in filtered_content}]. Found {expected_output} in filtered_content.')
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            futures = {
                executor.submit(
                    ...
                    remove_docker_image,  # rm_image (bool): Whether to remove the image after running. default: False
                    force_rebuild,
                    client,
                    run_id,
                    timeout,
