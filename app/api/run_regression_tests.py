...
from app.api.run_tests import run_tests
...
def _run_regression_2(
    ...
    """
    Run regression tests without using argparse.

    Args:
        run_id: Unique identifier for the run
        predictions_path: Path to predictions file with normalized patches
        output_file: Output file path
        regression_tests: Path to regression tests file
        num_workers: Number of workers for parallel processing
        timeout: Timeout for running tests in seconds
        instance_ids: List of instance IDs to run
        load: Whether to load existing results
    ...
    if predictions_path == "gold":
        ...
        assert predictions_path.endswith(".jsonl"), "predictions_path must end with .jsonl"
        with open(predictions_path, "r") as file:
            data_lines = [json.loads(line) for line in file]

        if not load:
            # Create a simple object to mimic args structure
            run_args = {
            ...
            run_regression_for_each_instance(type('Args', (), run_args), data_lines, run_id)

        regression_dict = {}
        instance_test_dict = {}

        with open(regression_tests, "r") as file:
            for line in file:
                json_obj = json.loads(line.strip())
                instance_id = json_obj["instance_id"]
                test = json_obj["tests_passing_in_original_repo"]
        ...
        for data in data_lines:
            instance_id = data["instance_id"]
            if os.path.isfile(
                    f"logs/run_evaluation/{run_id}/test/{instance_id}/report.json"
            ):
                regression_dict[instance_id] = rewrite_report(
                    instance_id,
                    f"logs/run_evaluation/{run_id}",
                    instance_test_dict,
                )
            else:
                regression_dict[instance_id] = []
        ...
        output_path = predictions_path.replace(
            ".jsonl", "regression_test_results.jsonl"
        )
        ...
        # Generate list of passing tests from original repo
        ds = load_dataset(dataset)
        instance_ids = ds["test"]["instance_id"] if instance_ids is None else instance_ids

        # Feed in list of blank patches
        patches = [
            {"instance_id": instance_id, "patch": "", "normalized_patch": ""}
            for instance_id in instance_ids
        ]
        ...
        if regression_tests:
            ...
            save_passing_tests(
                output_file,
                os.path.join("logs", "run_evaluation", run_id),
                dataset,
            )
        return output_file
