...
def run_test_suite_for_correctness(
    ...
    with app_utils.cd(project_path):
        try:
            cp = app_utils.run_string_cmd_in_conda(
                logger,
                test_cmd if not is_pre_test else pre_test_cmd,
                env_name,
    ...
    pass2pass_true = True
    if len(eval_result["PASS_TO_PASS"]["failure"]) != 0:
        pass2pass_true = False
    ...
    if resolution_status == ResolvedStatus.FULL:
        ...
        return False, error_message, pass2pass_true
