...
from subprocess import PIPE, STDOUT, TimeoutExpired, CalledProcessError
...
class ProjectApiManager(object):
    ...
    def __init__(
        ...
        self.env_name = env_name.replace('setup_', '')
        ...
        self.pre_test_cmd: str = pre_test_cmd
        self.is_pre_test: bool = is_pre_test
        ...
        with apputils.cd(self.project_path):
            print('Reseting repo in project_path', self.project_path)
            apputils.repo_reset_and_clean_checkout(self.commit, self.logger)
            print('Init repo in project_path', self.project_path)

        # Install task-specific dependencies;
        if do_install and not is_pre_test:
            self.do_install()
    ...
    def modify_setuptools_version(self, filename):
        import toml
        ...
        try:
            ...
            return
        # modify 'setuptools' to avoid environment error
        if 'build-system' in data and 'requires' in data['build-system']:
            requirements = data['build-system']['requires']
            for i, req in enumerate(requirements):
                if req == 'setuptools':
                    requirements[i] = 'setuptools==69.5.1'

        # write to src filename
    ...
    def do_install(self):
        ...
        with apputils.cd(self.project_path):
            ...
            if "astropy" in self.task_id:
                self.modify_setuptools_version("pyproject.toml")
    ...
    def do_install_swe_bench(self):
        ...
        if not self.pre_install_cmds and not self.install_cmd:
            # no command for installation, skip
            return
        # env_name = self.env_name.replace('setup_', '')
        with apputils.cd(self.project_path):
            # (0) For matplotlib, qhull tarball download
            # just fails, so we need to pre-install the system version and use it
            if "matplotlib" in self.task_id:
    ...
    def apply_test_patch(self):
        ...
        # if self.is_pre_test:
        self.test_patch = NOOP_PATCH.format(nonce="test_patch")
        ...
        with apputils.cd(self.project_path):
            ...
            # (2) check and remove existing file
            existing_file = pjoin(self.project_path, f"empty.file.test_patch.ignore")
            ...
            try:
                ...
                if cp.returncode != 0:
                    log_and_print(self.logger, cp.stderr)
                    raise RuntimeError(f"Command {apply_cmd} failed with return code {cp.returncode}.")
            except CalledProcessError as e:
                log_and_print(self.logger, f"Command {apply_cmd} failed with error: {e.stderr}")
                raise

            # (3) remove the temp file, which is not so important
            os.remove(test_patch_path)

        print('[apply test_patch] done.')
    ...
    def mcts_code_search(self, issue):
        ...
        global_save_path = f"SWESynInfer/experiment/global_dir"
        local_global_save_path = f"SWESynInfer/experiment/global_dir"
        ...
        if globals.only_mcts:
            global_load_path = None
        # global_load_path = None
        search_result, mcts_cost = run_mcts(issue, repo_path, global_save_path, instance_id, global_load_path,
                                            graph_load_path)
        return search_result, mcts_cost
    ...
    def write_patch(self, message_thread: MessageThread) -> Tuple[str, str, bool]:
        ...
        if not os.path.exists(self.project_path):
            self.project_path = self.project_path.replace("/opt",
                                                          '/Users/mayingwei/Documents/code/QRepoAgent/examples/devin_swe/local_docker/opt')
