...
def convert_json2pure_json(content):
    # 检查是否以 ```json\n 开头，以 ``` 结尾
    if content.startswith('```json\n') and content.endswith('```'):
        # 去掉开头的 ```json\n 和结尾的 ```
        pure_json_str = content[8:-3].strip()
        
        # 尝试解析 JSON 字符串
        try:
            json_obj = json.loads(pure_json_str)
            return pure_json_str
        except json.JSONDecodeError:
            print("Proxy Error: Invalid JSON format")
            return pure_json_str
    else:
        # 如果不符合预期格式，返回原始内容
        return content
...
def run(
    ...
    res_text = convert_json2pure_json(res_text)
