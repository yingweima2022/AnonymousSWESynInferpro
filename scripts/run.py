...
def run_agent(
    ...
    if not print_more:
        cmd += "--no-print "
    if multi_patch_sample:
        cmd += "--multi-patch-sample "

    print(f"Running agent workflow with cmd: {cmd}")
    with cd(root_dir):
        cp = run_string_cmd_in_conda(cmd, "swesyninfer", env=modified_env)
...
def main():
    ...
    multi_patch_sample = config.getboolean("DEFAULT", "multi_patch_sample", fallback=False)
    ...
    print(f"Experiment {expr_id} done.")
